...
int main(void)
{
	...
	game.box[0].center.x = 120 + 5*65 - 300;
	game.box[0].center.y = 500 - 5*60 + 350;
	game.box[1].width = 100;
	game.box[1].height = 10;
	game.box[1].center.x = 120 + 5*65 - 240;
	game.box[1].center.y = 500 - 5*60 + 310;
	game.box[2].width = 100;
	game.box[2].height = 10;
	game.box[2].center.x = 120 + 5*65 - 180;
	game.box[2].center.y = 500 - 5*60 + 270;
	game.box[3].width = 100;
	game.box[3].height = 10;
	game.box[3].center.x = 120 + 5*65 - 120;
	game.box[3].center.y = 500 - 5*60 + 230;
	game.box[4].width = 100;
	game.box[4].height = 10;
	game.box[4].center.x = 600;
	game.box[4].center.y = /*500 - 5*60 + */100;
	//game.circle.center.x = 600; 
	//game.circle.center.y = 500 - 5*60 + 350;//50
	//game.circle.radius = 50; //100
...
int check_keys(XEvent *e, Game *game)
{
	...
	if (e->type == KeyPress) {
		...
		if (key == XK_Right) {
			game->character.center.x += 2.5;
}
		if (key == XK_Left) {
			game->character.center.x -= 15;
}
		if (key == XK_Up) {
			game->character.center.y += 50;
}
...
void movement(Game *game)
{
	...
	for(int i = 0; i<=100/*game->n*/; i++) { //circle gravity stuff.....need game->n++, currently linked to makeParticle, if it's not running, then it won't run
	    p = &game->particle[i];
	    game->circle.center.x == 10; //p->velocity.x; 
	    game->circle.center.y += -0.01; //-0.001
	    //p->velocity.y -= GRAVITY;
		
		}  
	for(int j = 0; j<5; j++) {//added
		Shape *s = &game->box[j];
	//character gravity and (boundry?)
	for(int i = 0; i<=100; i++) { 

	c = &game->character;
	if(game->character.center.y < s->center.y + s->height) {
	game->character.center.y == 0;
	break;
}
	game->character.center.x == 0;
	game->character.center.y += -0.02;
}//added
...
//	for(int i = 0; i<=game->n; i++) {
	...
	for(int i = 0; i<game->n; i++) {
	    p = &game->particle[i];
	    p->s.center.x == p->velocity.x; //+= particles go right -= go left == centered
	    p->s.center.y += p->velocity.y; //-= particles go up
	    p->velocity.y -= GRAVITY; 
...
/*	//check circle collision
	...
	if(dist <= game->circle.radius) {
	    //p->velocity.y = 0.0;//-p->velocity.y;
	    //float v[2];
	    //v[0] = p->center.x - game->circle.center.x;
	    d0 /= dist;
	    d1 /= dist;
	    ...
	    p->velocity.x += d0 * 0.008;
	    p->velocity.y += d1 * 0.008; */
	}
		//check circle collision
	float d0,d1,dist;
	d0 = p->s.center.x - game->circle.center.x;
